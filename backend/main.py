...
async def upload_document(
    ...
    if ext not in allowed:
        raise HTTPException(400, f"Tipo de archivo no permitido. Use: {', '.join(allowed)}")
    ...
    return {"message": "Documento subido e indexado", "doc_id": doc.id, "filename": file.filename}
    ...
    return [
        {
            ...
            "status": d.status, "area": d.area, "description": d.description,
...
def chat(request: ChatRequest, current_user: User = Depends(get_current_user)):
    result = query_rag(request.question, request.doc_ids)
    return result
